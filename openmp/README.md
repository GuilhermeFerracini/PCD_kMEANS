...
2. **Compilação e Execução**:
   - Abra o notebook `.ipynb` deste diretório no VS Code ou Jupyter Lab.
   - O notebook contém comandos para compilar e executar o código C.
   - O comando de compilação típico (via célula de código) será:
     ```python
     !gcc -o kmeans_omp kmeans_omp.c -fopenmp
     ```
   - Em seguida, o código é executado chamando o binário gerado (ex: `!kmeans_omp.exe`).
